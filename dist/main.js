...
module.exports = function BakeEnd(options) {
  var _options$mongoURL = options.mongoURL,
    ...
    gqlMiddleware = _options$gqlMiddlewar === undefined
  ...
  var gqlSchema = (0, _schema.autoGenerateGraphQLSchema)(
    dataRequirements,
    dbModels
  );
  ...
  app.use(
    endpointURL,
    middlewares,
    (0, _graphqlServerExpress.graphqlExpress)(function(req, res, next) {
  ...
  if (showGraphiql)
    app.use(
      "/graphiql",
      (0, _graphqlServerExpress.graphiqlExpress)({ endpointURL: endpointURL })
    );
  ...
  return {
    ...
    schema: gqlSchema,
    run: function run() {
      var port = arguments.length > 0 && arguments[0] !== undefined
        ? arguments[0]
        : 4000;
