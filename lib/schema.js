  ...
  Object.keys(ObjectTypes).forEach(objectName => {
    ...
    getQueries[queryName] = {
      ...
      resolve: async function (_, {...args}, { permissions }) {
        return can(objectName, queryType, permissions) ? await modelObject.model.findOne(sanitizeArgs(args)) : permissionDenied
      }
    }
...
function generateListQueries(ObjectTypes, modelObjects) {
  ...
  Object.keys(ObjectTypes).forEach(objectName => {
    ...
    listQueries[queryName] = {
      ...
      resolve: async function (_, { pageSize, pageNumber, ...args }, { permissions }) {
        return can(objectName, queryType, permissions) ? await modelObject.model.find(sanitizeArgs(args)) : permissionDenied
      }
    }
  ...
  Object.keys(ObjectTypes).forEach(objectName => {
    ...
    createMutations[mutationName] = {
      ...
      resolve: async function (_, { ...args }, context) {
        return can(objectName, mutationType, context) ? await modelObject.model(args).save() : permissionDenied
      }
    }
  ...
  Object.keys(ObjectTypes).forEach(objectName => {
    ...
    updateMutations[mutationName] = {
      ...
      resolve: async function (_, { id, ...args}, { permissions }) {
        return can(objectName, mutationType, permissions) ? await modelObject.model.findOneAndUpdate({ id: id }, args, { new: true }) : permissionDenied
      }
    }
  ...
  Object.keys(ObjectTypes).forEach(objectName => {
    ...
    removeMutations[mutationName] = {
      ...
      resolve: async function (_, { ...args }) {
        return can(objectName, mutationType, permissions) ? await modelObject.model.remove(args) : permissionDenied
      }
    }
